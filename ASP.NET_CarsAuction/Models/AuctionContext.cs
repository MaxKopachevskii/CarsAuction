...
namespace ASP.NET_CarsAuction.Models
{
    ...
    public class AuctionInitialaizer : DropCreateDatabaseAlways<AuctionContext>
    {
        protected override void Seed(AuctionContext db)
        {
            ...
            db.Categoties.Add(category1);
            db.Categoties.Add(category2);
            db.Categoties.Add(category3);
            db.SaveChanges();
            ...
            Car car7 = new Car { Name = "Honda NSX", ShortDesc = "Cпортивный автомобиль со среднемоторной компоновкой", LongDesc = "Cпортивный автомобиль со среднемоторной компоновкой, производившийся компанией Honda с 1990 по 2005 г. В Северной Америке и Гонконге продавался под маркой Acura. При проектировании было использовано множество новаторских решений: это первый автомобиль, кузов и шасси которого изготовлены из алюминия, оснащался полностью электронным усилителем руля, электронной педалью газа (так называемый Drive-by-Wire), двигатель имел V-образное расположение цилиндров, максимальное количество оборотов 8000 в минуту.", Price = 25000, Img = "https://auto.vercity.ru/img/magazine/2020/02/29/1583003352.jpg", CategoryId = category2.Id,  IsCheck = true };
